...
```{r}
d_2019 <- read_xlsx("raw_data/class_enrollment_summary_by_term_12.20.19.xlsx", skip = 3) %>%
  clean_names() %>%
  select(id= course_id, title= course_title, name=course_name, department=course_department, u_grad) %>%
filter(u_grad > 10)
d_2019
```
...
```{r}
read_enrollment <- function(file, rows){
  read_xlsx(file, skip = rows) %>%
  clean_names() 
}
...
```{r}
d_2019 <- read_enrollment("raw_data/class_enrollment_summary_by_term_12.20.19.xlsx", 3)
d_2018 <- read_enrollment("raw_data/class_enrollment_summary_by_term_10.24.18.xlsx", 2)
d_2017 <- read_enrollment("raw_data/class_enrollment_summary_by_term_10.20.17.xlsx", 3)
d_2016 <- read_enrollment("raw_data/class_enrollment_summary_by_term_fall_2016.xlsx", 0)
d_2015 <- read_enrollment("raw_data/class_enrollment_summary_by_term_fall_2015.xlsx", 3)
```
...
```{r}
bind_rows(d_2019, d_2018, d_2017, d_2016, d_2015)
```
